...
class Categories5ka(Parse5ka):
    ...
    def run(self):
        for cat in self.get_categories():
            result_cat = {'name': cat['parent_group_name'],
                         'code': cat['parent_group_code'],
                         'products': list(self._parse('https://5ka.ru/api/v2/special_offers/?categories='+cat['parent_group_code']))}
            print(result_cat)
            self._save(result_cat, self.save_path.joinpath((
